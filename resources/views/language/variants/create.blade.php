...
@section('title', 'Nueva Variante de Idioma')
...
<h4 class="py-3 breadcrumb-wrapper mb-4">
  <span class="text-muted fw-light">Variantes de Idioma /</span> Crear Nueva
</h4>

<div class="row">
  <div class="col-md-12">
    <div class="card mb-4">
      <h5 class="card-header">Nueva Variante de Idioma</h5>
      <div class="card-body">
        <form action="{{ route('language-variants.store') }}" method="POST">
          @csrf
          ...
          <div class="row mb-3">
            <div class="col-md-6">
              ...
              <input type="text" class="form-control @error('country_code') is-invalid @enderror" 
                    ...
                    value="{{ old('country_code') }}" maxlength="2">
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    // Auto-fill code when base language is selected
    $('#base_language').on('change', function() {
      const baseCode = $(this).val();
      if (baseCode) {
        $('#code').val(baseCode + '_');
        $('#country_code').val(baseCode);
        $('#flag').val(baseCode);
      }
    });
  });
