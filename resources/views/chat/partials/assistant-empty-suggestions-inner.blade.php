{{-- Suggested prompts align with App\Services\AssistantToolsService --}}
<div class="assistant-empty-suggestions">
    <p class="text-muted small mb-3 mb-md-2">
        Puedes pedir al asistente acciones reales sobre tu equipo. Ejemplos (clic para poner el texto abajo):
    </p>
    <div class="row g-2 small">
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-users me-1"></i>Contactos</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Crea un contacto llamado Ana López con teléfono 34600111222">Crear contacto</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista las categorías de contactos">Categorías</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-chart-bar me-1"></i>Informes</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Dame un resumen de mi cuenta">Resumen</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista mis contactos recientes">Contactos</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista mis tareas recientes">Tareas</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-checkbox me-1"></i>Tareas</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Crea una tarea: llamar al cliente mañana">Crear tarea</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista los miembros del equipo">Equipo (asignar)</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-brand-whatsapp me-1"></i>WhatsApp</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Envía un WhatsApp al 34600111222 con el mensaje: Hola, te escribo desde Humano">Enviar mensaje</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-calendar me-1"></i>Calendario</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="¿Qué eventos tengo esta semana?">Listar</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Agenda una reunión mañana a las 15:00 con título Seguimiento cliente">Agendar</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-ticket me-1"></i>Tickets</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Abre un ticket: no puedo enviar la campaña de hoy">Abrir ticket</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-mail me-1"></i>Plantillas email</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista las plantillas de email del equipo">Listar plantillas</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-speakerphone me-1"></i>Campañas (News)</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Lista las campañas o mensajes de News">Listar campañas</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-shopping-cart me-1"></i>Catálogo</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Muéstrame el catálogo de productos para WhatsApp">Catálogo</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Busca productos que contengan camiseta">Buscar</button>
                </div>
            </div>
        </div>
        <div class="col-12 col-md-6">
            <div class="border rounded p-2 h-100 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-user me-1"></i>Perfil</strong>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Muéstrame mi perfil y mi equipo actual">Mi perfil</button>
                </div>
            </div>
        </div>
        <div class="col-12">
            <div class="border rounded p-2 bg-label-secondary bg-opacity-10">
                <strong class="d-block mb-1"><i class="ti ti-route me-1"></i>Flujos del equipo</strong>
                <p class="text-muted mb-2 mb-md-1 small">Si tu equipo configuró flujos (facturas, catálogo, agendar…), dilo con claridad y el asistente continuará en ese contexto.</p>
                <div class="d-flex flex-wrap gap-1">
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Necesito pagar o consultar mis facturas">Facturación</button>
                    <button type="button" class="btn btn-sm btn-label-secondary py-0 px-2 assistant-suggestion-example" data-prompt="Quiero ver el catálogo para comprar por WhatsApp">Compras</button>
                </div>
            </div>
        </div>
    </div>
    <p class="text-muted small mt-3 mb-0">
        <i class="ti ti-terminal ti-xs me-1"></i>
        Opcional:
        @if (! empty($selectedPhone ?? null))
            <code>php artisan chat:simulate --phone={{ $selectedPhone }}</code>
        @else
            <code>php artisan chat:simulate</code>
        @endif
        — misma sesión que este chat si usas el mismo usuario en la terminal.
    </p>
</div>
