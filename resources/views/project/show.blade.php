...
<div class="card mb-4">
	<div class="card-header d-flex justify-content-between align-items-center">
		<h5 class="mb-0"><i class="ti ti-clock me-2"></i>{{ __('Time Tracking') }}</h5>
		<a href="{{ route('time.index') }}?project_id={{ $project->id }}" class="btn btn-sm btn-outline-primary">
	...
	<div class="card-body">
			...
			<div class="col-md-4">
				<div class="d-flex align-items-center">
					...
					<div>
						...
						<h5 class="mb-0">{{ number_format($totalHours ?? 0, 2) }}h</h5>
					</div>
				</div>
			</div>
			<div class="col-md-4">
				<div class="d-flex align-items-center">
					...
					<div>
						<small class="text-muted d-block">{{ __('Team Members Tracking') }}</small>
						<h5 class="mb-0">{{ $timeEntries->pluck('user_id')->unique()->count() }}</h5>
					</div>
				</div>
			</div>
			<div class="col-md-4 text-end">
				@can('time.create')
					<a href="{{ route('time.create') }}?project_id={{ $project->id }}" class="btn btn-primary">
						<i class="ti ti-plus me-1"></i>{{ __('Add Time Entry') }}
					</a>
				@endcan
			</div>
		</div>

		@if($timeEntries && $timeEntries->count() > 0)
					...
					<tbody>
						@foreach($timeEntries as $entry)
							<tr>
								...
								<td>{{ $entry->start_time->format('d/m/Y H:i') }}</td>
								<td>
									@if($entry->isRunning())
										<span class="badge bg-label-success">{{ __('Running') }}</span>
									@else
								...
								<td>
									@if($entry->is_billable)
										<span class="badge bg-label-primary">{{ __('Billable') }}</span>
									@else
										<span class="badge bg-label-secondary">{{ __('Non-billable') }}</span>
									@endif
								</td>
							</tr>
						@endforeach
		...
		@else
			<div class="text-center py-4">
				<i class="ti ti-clock-off ti-lg text-muted mb-2"></i>
				<p class="text-muted">{{ __('No time entries yet for this project') }}</p>
			</div>
		@endif
	</div>
</div>
