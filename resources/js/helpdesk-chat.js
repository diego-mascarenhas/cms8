$(document).ready(function () {
	...
	function sendMessage(message) {
		...
		$.ajax({
			...
			error: function (xhr, status, error) {
				botMessageElement.find('.chat-message-text img').remove();
				botMessageElement
					.find('.chat-message-text p')
					.html('Error: Either you have not entered your API key or GPT is not working at this time.');
			},
			complete: function () {
				scrollToBottom();
			},
	...
	document.querySelector('.form-send-message').addEventListener('submit', e => {
		...
		if (document.querySelector('.message-input').value) {
			...
			renderMsg.innerHTML =
				'<p class="mb-0 text-break">' + document.querySelector('.message-input').value + '</p>';
