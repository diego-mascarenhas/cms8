...
use App\Sync\Providers\WebDavTaskSyncProvider;
use Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Foundation\Queue\Queueable;
use Illuminate\Support\Facades\Log;

class SyncWebDavTasksJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(WebDavTaskSyncProvider $provider): void
    {
        $account = ExternalAccount::query()->with('team')->find($this->externalAccountId);

        if ($account === null || $account->team === null || ! $account->team->webdavTasksInboundSyncEnabled())
        {
            return;
        }
        ...
        {
            Log::error('WebDAV task sync failed.', [
                'external_account_id' => $account->id,
