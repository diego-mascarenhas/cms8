...
use App\Sync\Providers\WebDavContactSyncProvider;
use Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Foundation\Queue\Queueable;
use Illuminate\Support\Facades\Log;

class SyncWebDavContactsJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(WebDavContactSyncProvider $provider): void
    {
        $account = ExternalAccount::query()->with('team')->find($this->externalAccountId);

        if ($account === null || $account->team === null || ! $account->team->webdavContactsInboundSyncEnabled())
        {
            return;
        }
        ...
        {
            Log::error('WebDAV contact sync failed.', [
                'external_account_id' => $account->id,
