...
use App\Sync\Providers\WebDavCalendarSyncProvider;
use Illuminate\Contracts\Queue\ShouldQueue;
use Illuminate\Foundation\Queue\Queueable;
use Illuminate\Support\Facades\Log;

class SyncWebDavCalendarEventsJob implements ShouldQueue
{
    use Queueable;
    ...
    public function handle(WebDavCalendarSyncProvider $provider): void
    {
        $account = ExternalAccount::query()->with('team')->find($this->externalAccountId);

        if ($account === null || $account->team === null || ! $account->team->webdavCalendarInboundSyncEnabled())
        {
            return;
        }
        ...
        {
            Log::error('WebDAV calendar sync failed.', [
                'external_account_id' => $account->id,
