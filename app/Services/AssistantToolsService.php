...
class AssistantToolsService
{
    ...
    private function listProductCatalog(int $teamId, array $input): string
    {
        ...
        $lines[] = 'To buy from WhatsApp: use add_to_whatsapp_cart with product_id or product_code, or tell the customer they can write: comprar [name or code], or agregar [n] [name] (e.g. add multiple units). Then: carrito, quitar [n] [name] or quitar todo [name], finalizar to close the order (pagar/cerrar pedido/checkout also work in the bot; suggest only finalizar to the customer).';
    ...
    private function addToWhatsAppCart(int $teamId, array $input): string
    {
        ...
        if ($this->contextCustomerPhone === null || $this->contextCustomerPhone === '')
        {
            return 'Cannot attach to a WhatsApp cart: no customer phone in this session. Tell the customer to write from WhatsApp, or use the commands: comprar [name or code], agregar [n] [name], carrito, quitar [n] [name], finalizar. If you are in the web assistant with a recipient phone, open the assistant with that recipient selected.';
        }
        ...
        $msg .= 'Tell them they can write *carrito* to review, *quitar [n] [product]* to remove units, or *finalizar* to finish the order.';
