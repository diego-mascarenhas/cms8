...
class AstralChartService
{
    ...
    private function generateInterpretation($zodiacData, $northNodeData, $birthDate, $countryName = null): string
    {
        ...
        if (empty($apiKey))
        {
            \Log::info('Anthropic API not configured, using fallback interpretation');
        ...
        try
        {
            ...
            $response = $this->sendClaudeRequest($prompt);

            // Extract interpretation
            $interpretation = $response['content'][0]['text'] ?? null;
        ...
        {
            \Log::warning('Claude API request failed: '.$e->getMessage());
        }
    ...
    /**
     * Send request to Claude API with token logging
     */
    private function sendClaudeRequest(string $prompt): array
    {
        // Prepare payload
        $payload = [
        ...
        {
            \Log::warning('Toon encoding failed, using JSON: '.$e->getMessage());
            $useToon = false;
            $toonSize = $jsonSize;
        ...
        $response = Http::withHeaders([
            ...
            ->timeout(config('anthropic.timeout', 30))
            ->post(config('anthropic.api_url', 'https://api.anthropic.com/v1').'/messages', $payload);
