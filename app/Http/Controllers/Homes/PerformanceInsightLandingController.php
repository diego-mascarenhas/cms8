...
class PerformanceInsightLandingController extends Controller
{
    public function index(): View
    {
        return view('homes.performance-insight.landing', [
            'presentationUrl' => GuidePresentation::url('insight-diario'),
        ]);
    }

    public function newsletter(): View
    {
        return view('homes.performance-insight.newsletter', [
            'landingUrl' => route('performance-insight.landing'),
            'presentationUrl' => GuidePresentation::url('insight-diario'),
            'registerUrl' => route('performance-insight.landing').'#empezar',
        ]);
    }
    ...
    public function storeLead(StoreSlashLandingLeadRequest $request): RedirectResponse
    {
        ...
        $sourceLabel = __('performance_insight_landing.lead.sources.'.$source);
        ...
        {
            Log::channel('leads')->warning('Performance Insight landing lead notification skipped: NOTIFICATION_EMAIL is not configured.', [
                'email' => $email,
                'source' => $source,
            ]);
        }
        ...
        Log::channel('leads')->info(sprintf(
            '[%s] Performance Insight landing lead - Email: %s, Name: %s, Phone: %s, Source: %s',
            $submittedAt,
            $email,
        ...
        return redirect()
            ->to(route('performance-insight.landing').'#empezar')
